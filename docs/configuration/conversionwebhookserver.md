...
| `startupProbe.{enabled,periodSeconds,failureThreshold}` | The cold-start budget: `periodSeconds × failureThreshold`, defaulting to `5 × 60` (five minutes). A replica does not listen on any port until its registry has compiled every assigned plan, so without a `startupProbe` the liveness probe's own `3 × 10 s` would be the whole budget and a replica slower than that would crash-loop forever. See [Capacity planning](../operations/capacity.md#cold-start-how-long-before-a-replica-can-serve). |
