...
| **13 — CI/CD: official GitHub Actions** | First-party composite Actions (`setup-convctl` with signature verification, `convctl-test`, `convctl-diff`, `convctl-fleet`) **with their own test workflow** — cross-runner matrix, a negative test proving verification fails on a tampered artifact, and assertions on annotations and job summaries, not just exit codes. Plus a published `convctl` image, CI-native output formats (GitHub annotations, SARIF, markdown), `convctl lint` over a whole repo, and distribution via Homebrew / krew / deb / rpm. |
