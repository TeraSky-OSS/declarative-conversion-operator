...
## CLI support for testing native CRDs offline

`convctl` today only loads XRD YAML (`--xrd`) for its offline `validate`/`analyze`/`test` commands — there's no `--crd` equivalent yet for testing a `CRDConversionConfig` against a local `CustomResourceDefinition` YAML file the same way. Since `pkg/crdadapter` already implements the exact same `SchemaSource` interface `pkg/xrdadapter` does, this is a small, contained CLI-only addition once there's demand for it.
...
- Additional `convctl test` output formats beyond `table`/`json` (e.g. JUnit XML for CI integration).
- A `convctl diff` style command to preview exactly what an `XRDConversionConfig` update would change about an existing plan's coverage, before applying it.
