...
- **Crossplane 2.x only.** The operator reads XRDs at `apiextensions.crossplane.io/v2`, which a Crossplane 1.x control plane does not serve — 1.x clusters are out of scope, not merely untested. The **v1 compatibility layer inside Crossplane 2.x is in scope**: `scope: LegacyCluster` XRDs (declared through the still-served `apiextensions.crossplane.io/v1` XRD API) put their Crossplane machinery fields directly under `spec` rather than under `spec.crossplane`, and the conversion engine passes both layouts through unchanged.
