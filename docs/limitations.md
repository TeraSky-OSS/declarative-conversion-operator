...
- **`forEach` nesting is capped at depth 1.** A `forEach` rule's own rule list can't contain another `forEach` — this bounds the CRD schema recursion the engine has to reason about. See [For Each](strategies/for-each.md).
- **`forEach` requires strict positional correspondence** between the hub and spoke arrays — same length, same order. A length mismatch at conversion time is a hard runtime error, not a best-effort merge.
