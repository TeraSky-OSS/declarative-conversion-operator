...
- **No cross-cluster or multi-region coordination.** Every `ConversionWebhookServer` instance and every operator replica assumes a single Kubernetes cluster. A webhook-server in cluster A serving conversions for a resource in cluster B is an explicit non-goal — see [Architecture: One cluster, one install](architecture.md#one-cluster-one-install). Keep configs consistent across a fleet with `convctl test --live` / `diff --live` per kubecontext, not by sharing webhook endpoints.
