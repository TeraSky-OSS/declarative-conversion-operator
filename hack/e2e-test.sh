...
assert_eq "$(kubectl get "${V3}" "${NAME_V1}" -n "${COMPOSITE_NS}" -o jsonpath='{.spec.packed}')" "784" "CEL (s2h): spec.bitHigh 3 * 256 + spec.bitLow 16 -> spec.packed 784"
...
assert_eq "$(kubectl get "${V1}" "${NAME_V3}" -n "${COMPOSITE_NS}" -o jsonpath='{.spec.bitHigh}')" "4" "CEL (h2s): spec.packed 1025 / 256 -> spec.bitHigh 4"
assert_eq "$(kubectl get "${V1}" "${NAME_V3}" -n "${COMPOSITE_NS}" -o jsonpath='{.spec.bitLow}')" "1" "CEL (h2s): spec.packed 1025 % 256 -> spec.bitLow 1"

log "All e2e conversion checks passed (every one of pkg/engine's 29 built-in strategies exercised against a real apiserver)"
